...
companies %>% 
  ...
  labs(
    title = "More computer than pharma companies are sampled.",
    x = "Type of Company",
    y = "Number of Companies Sampled",
    fill = "Type of Company"
  )
```
...
1. Scale (standard deviation and inter quartile range)
...
## Use filter to select rows, in this case the computer companies.
```{r filter companies}
companies %>% 
  filter(type == "Computer")
```

## Use select to decrease the number of columns, in this case the company type and profit_per_employee
...
```{r function that calls a function}
summary_function <- function(FUN = mean){
  important_variables %>%
    map_dbl(FUN)
}

summary_function() # calls the mean by default
summary_function(median)
summary_function(FUN = IQR)
```
...
companies %>% # automatic and temporary conversion to a factor
  count(size)
...
companies %>% 
  ...
  labs(
    title = "A majority of sampled companies are small.",
    x = "",
...
## Now consider the effect on the bar chart of reordering inherent in the creation of the factor.
...
## Add population to the counties sf object
...
days_to_christmas <- function(){
  number_of_days <- (today() %--% mdy(12252020)) / days(1)
