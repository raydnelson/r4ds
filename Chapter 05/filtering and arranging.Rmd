...
### Use a boxplot plot to compare the departure delays for the three different airports for New Years Day flights 
...
three_airports <- flights %>% filter(dest %in% c("ATL", "LAX", "ORD"), arr_delay < 120)
...
# Boxplot
three_airports %>% ggplot(aes(x = dest, y = arr_delay)) +
  geom_boxplot(fill = "lightgreen") +
  coord_flip()
```  
