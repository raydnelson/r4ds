...
```{r airport average median delay, warning = FALSE}
...
## Smoothed scatterplot of average distance and average delay for each destination location

```{r distance and average arrival delay, message = FALSE, warning = FALSE}
...
## Smoothed scatterplot of average distance and average delay for each location using piping

```{r distance and average arrival delay using piping, message = FALSE, warning = FALSE}
flights %>%
  group_by(dest) %>%
  summarize(
    count = n(),
...
## Piping to get average and median delay for each airport

```{r piping to compare airports, warning = FALSE}
flights %>%
  group_by(origin) %>%
  summarize(
    mean_delay = mean(dep_delay, na.rm = TRUE),
