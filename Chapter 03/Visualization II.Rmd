...
## Distribution geometric objects
...
ggplot(data = mpg, mapping = aes(x = class, y = hwy, fill = class)) +
  geom_violin(show.legend = FALSE) +
  geom_boxplot(mappingshow.legend = FALSE, width = 0.05, fill = "grey")
```  
...
crimesm <- gather(crimes, key = variable, value = value, Murder,
                  Assault, UrbanPop, Rape, -state)
crimesm$variable <- as.factor(crimesm$variable)
if (require(maps)) {
