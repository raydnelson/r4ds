---
title: "Vectors"
author: "MPA 634: Data Science for Managers"
date: "6 November 2019"
output: 
  html_document:
    code_folding: "hide"
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE,
                      message = FALSE,
                      warning = FALSE)
# Load libraries
library(tidyverse)
```

## How to store and access information in R. We have the following possible objects:  

*  atomic vectors (1 dimensional: character, logical, and numeric)
*  lists
*  tibbles (which are a special case of lists)  

## Create vectors in which we place information 

### Individuals associated with MPA 634 (past and present students)

```{r individuals associated with each section}
individuals_current <- c(
  "Brooke",
  "Tessa",
  "Pallavi",
  "Alex",
  "Oogii",
  "Sarah",
  "Joshua",
  "Stan",
  "Haley",
  "Erica",
  "Mitchell",
  "Justin",
  "Haris",
  "David",
  "Vana"
)

individuals_previous <- c(
  "Derrik",
  "Devin",
  "Mike",
  "Filipe",
  "Kiersten",
  "Ruben",
  "Jonathan",
  "Matt",
  "Teresa",
  "Nate",
  "Ryan",
  "Alex",
  "Bruno",
  "Kyle",
  "Chris",
  "Taylor",
  "Trevor",
  "Travis",
  "Sarah",
  "Chad",
  "Jonathan",
  "Seth",
  "Amanda",
  "Joshua",
  "Nathan"
)
```

### Programs in which individuals can participate  

```{r programs}
program_names <- c("MISM", "MBA", "MPA")
program_codes <- c(1:3)
```

### Programs that correspond to each individual  

```{r individuals in programs}
programs_current <- c(1, 3, 2, 3, 3, 3, 1, 3, 1, 3, 1, 1, 3, 1, 3)
programs_previous <- c(2, 2, 2, 2, 1, 1, 2, 2, 1, 3, 3, 2, 2, 1, 2,
                       1, 2, 2, 3, 2, 3, 2, 2, 2, 3)
```

### Types of the objections that we have created

```{r types of objects}
typeof(individuals_current)
typeof(programs_current)
typeof(program_names)
typeof(program_codes)
```

## Organize our information into tibbles and lists

### Programs as a tibble and a list

```{r programs as a tibbles and a list}
(programs <- tibble(program_names, program_codes))
(programs_list <- list(program_names = program_names, program_codes = program_codes))
```

### Individuals as tibbles and lists

```{r individuals as tibble I}
length(individuals_current)
length(individuals_previous)
```

### Error in creating tibble
```{r error in creating tibble, error = TRUE}
(classes_tibble) <- tibble(individuals_current, individuals_previous)
```

### Lists work because vectors of different lengths are allowed.
```{r lists work}
(class_list <- list(individuals_current = individuals_current,
                    individuals_previous = individuals_previous))
```

### Individuals as a long tibble

```{r long individual tibble}
(current <- rep("current", length(individuals_current)))
(previous <- rep("previous", length(individuals_previous)))

current <- tibble(individuals_current, programs_current, current)
colnames(current) <- c("individual", "program", "section")
current

previous <- tibble(individuals_previous, programs_previous, previous)
colnames(previous) <- c("individual", "program", "section")
previous

(classes <- rbind(current, previous))
```

##### Add program identifier to the tibble with inner join and cleanup the tibble
```{r}
classes %>% 
  inner_join(programs, by = c("program" = "program_codes")) %>% 
  select(-program) %>% 
  rename(program = program_names)
```

## Information about objects

```{r object information}
colnames(classes)
dim(classes)
nrow(classes)
ncol(classes)
str(classes)
```

## Accessing information in lists

### Objects for use in this exercise include:

*  classes (tibble that was already created)
*  programs (tibble that was already created)
*  individuals (list that needs to be created from individuals_previous and individuals_current)
*  a list called rdn_teaching which is a list that includes the above three objects

### Create the sections list

```{r sections list}
individuals <- list(individuals_current = individuals_current,
                    individuals_previous = individuals_previous)
``` 

### Type of the objects that we are goingindividuals_current rdn_teaching

```{r type of objects for rdn}
typeof(classes)
is_tibble(classes)
typeof(individuals)
is_tibble(individuals)
typeof(programs)
is_tibble(programs)
```

### Create the list rdn_teaching

```{r rdn_teaching creation}
rdn_teaching <- list(classes = classes, individuals = individuals, programs = programs)
```

### Investigate rdn_teaching

```{r rdn_investigation}
typeof(rdn_teaching)
is_tibble(rdn_teaching)
```

### Structure of the list  

```{r structure of rdn_teaching}
str(rdn_teaching)
```
### Accessing information in the first level of the list

```{r accessing elements in the rdn_list}
rdn_teaching$classes
rdn_teaching[[1]]
```

### Accessing information in a list that is within a list

```{r list within a list}
rdn_teaching$programs$program_names
rdn_teaching[[3]][[1]]
```  