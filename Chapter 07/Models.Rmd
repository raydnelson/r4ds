...
Our data exploration reveals that the relationship between price and diamond size is approximately linear. It also introduces the challenge that the variation in the price increases as the weight of the diamond increases. 
...
diamonds2 %>%
  ggplot(aes(x = reorder(cut, price, FUN = median), y = price)) +
...
```{r size by clarity}
diamonds2 %>%
  ggplot(aes(x = reorder(clarity, carat, FUN = median), y = carat)) +
  geom_violin(aes(fill = clarity), show.legend = FALSE) +
  geom_boxplot(width = 0.1, fill = "lightblue") +
...
```{r residuals by cut}
diamonds2 %>%
  ggplot(aes(x = reorder(cut, resid, FUN = median), y = resid)) +
  geom_violin(aes(fill = cut), show.legend = FALSE) +
  geom_boxplot(width = 0.1, fill = "lightblue") +
...
diamonds2 %>%
  ggplot(aes(x = reorder(clarity, resid, FUN = median), y = resid)) +
  geom_violin(aes(fill = clarity), show.legend = FALSE) +
