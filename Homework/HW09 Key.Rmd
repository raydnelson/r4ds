...
#### Extra.  Explain how Tukey's five number summary is related to the boxplot.

The boxplot contains each of the element that comprise Tukey's five number summary:

1. *Minimum* is either the smallest outlier or smallest value on the left or lower whisker.
...
5. *Maximum* is either the largest outlier or largest value included in the right or upper whisker. 
...
#### 3. Explain how you can use violin plots to determine whether a distribution has fat tail relative to the normal.
...
mpg %>%
  ggplot(aes(x = class %>% fct_reorder(cty, IQR), y = cty)) +
...
mpg %>% 
  ggplot(aes(y = drv %>% fct_reorder(hwy, median), x = hwy, fill = drv)) +
