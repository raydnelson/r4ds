...
```{r investigate how the scale function works, warning = FALSE}
x %>% scale() %>% t()
x %>%
  scale() %>%
  mean() %>%
...
```{r variables for violin plot of random normal observations for a variable, warning = FALSE}
# set.seed(Sys.time())
sample_size <- 100
mu <- 10
sigma <- 10
...
```{r diamond by cut, message = FALSE}
diamond_cut = "Ideal"
diamonds_subset %>% 
...
```{r price_weight_cut execution, message = FALSE}
